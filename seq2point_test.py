...
class Tester():
    ...
    def test_model(self):
        ...
        model.summary()
        test_generator = TestSlidingWindowGenerator(number_of_windows=self.__number_of_windows, inputs=test_input, targets=test_target, offset=self.__window_offset,qo=self._input_window_length%2)
        ...
        if tflite_test:
            ...
            for i in range(nthread):
                thread.append(tfThread(threadID=i,inputs=self.__test_input,outputs=self.__test_target,datadir=self.__datadir,appliance=self.__appliance,max_number_of_windows=self.max_number_of_windows,pre_data=pre_data,tar_data=tar_data))
            ...
            print("--MAE="+str(maesum/nthread/self.max_number_of_windows)+"; SAE="+str(saesum/nthread)+";--\n")
            logging.info("--MAE="+str(maesum/nthread/self.max_number_of_windows)+"; SAE="+str(saesum/nthread)+";--\n")
    ...
    def load_dataset(self, directory):
        ...
        test_target = np.round(np.array(data_frame.iloc[self.__window_offset: -self.__window_offset, 1], float), 6)  #seq2point
        del data_frame
    ...
    def plot_results(self, testing_history, test_input, test_target):
        ...
        metric_string = "SAE: {:}\n".format(nm.get_sae(test_target.flatten(), testing_history.flatten(), sample_second)) \
            + "SAEsita: {:}\n".format(nm.get_saesita(test_target.flatten(), testing_history.flatten(), sample_second)) \
            + 'NDE:{0}\n'.format(nm.get_nde(test_target.flatten(), testing_history.flatten())) \
            + '\n\n-----MAE: {:}\n    -std: {:}\n    -min: {:}\n    -max: {:}\n    -q1: {:}\n    -median: {:}\n    -q2: {:}\n' .format(*nm.get_abs_error(test_target.flatten(), testing_history.flatten())) \
            + 'Energy per Day: {:}\n'.format(nm.get_Epd(test_target.flatten(), testing_history.flatten(), sample_second))
        logging.info(metric_string)
        print(metric_string)
...
class tfThread (threading.Thread):
    ...
    def run(self):
        #--------------------------------Keras Model--------------------------------------------#
        # self.__crop
        interpreter = tf.lite.Interpreter(model_path="saved_models/" + self.__datadir + "_s/" + self.__appliance + "_model.tflite")
        ...
        for index in range(skipr, self.max_number_of_windows+skipr, 1):# + self.__window_offset
            ...
            output_data = ((output_data * appliance_data[self.__appliance]["std"]) + appliance_data[self.__appliance]["mean"])
            output_data[output_data < 0] = 0
            target_data = ((target_data * appliance_data[self.__appliance]["std"]) + appliance_data[self.__appliance]["mean"])
